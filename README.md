...
記憶體供應鏈情報追蹤。每天自動抓取供應鏈上下游公司的新聞與股價，用規則引擎標記重要性與異常，產出每日與 7 日報告，並在網頁上呈現供應鏈關係圖與事件時間軸。

線上看：<http://memory.intel.weiqi.kids/>

## 現在追蹤什麼

- 公司 62 家，分佈在上中下游，設定在 `configs/companies.yml`（含關聯的上下游對象，供應鏈圖就是從這裡長出來的）
- 主題 14 個，每個主題帶一組中英文關鍵字，設定在 `configs/topics.yml`

這是所有 intel 系列裡追蹤範圍最大的一個。
