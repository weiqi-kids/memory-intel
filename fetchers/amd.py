...
抓取來源：
- AMD IR RSS: https://ir.amd.com/news-events/press-releases/rss
"""
...
class AMDFetcher(CompanyFetcher):
    """AMD 官網爬蟲 - RSS"""
    ...
    def _parse_rss(self, xml_content: str) -> list[CompanyDocument]:
        ...
        try:
            ...
            for item in root.findall(".//item")[:30]:
                try:
                    ...
                    # 清理內容
                    clean_content = self._strip_html(description)[:500] if description else ""
                    ...
                    if 'hbm' in title_lower:
                        tags.append('hbm')
                    if any(kw in title_lower for kw in ['gpu', 'radeon', 'instinct']):
                        tags.append('gpu')
                    if any(kw in title_lower for kw in ['ryzen', 'epyc', 'cpu']):
                        tags.append('cpu')
                    if any(kw in title_lower for kw in ['ai', 'inference']):
                        tags.append('ai')
        ...
        logger.info(f"Parsed {len(documents)} news from AMD RSS")
        return documents

    def _parse_rss_date(self, date_str: str) -> Optional[datetime]:
        """解析 RSS 日期格式"""
        # AMD 格式: "Mon, 02 Mar 26 03:00:00 -0500"
        formats = [
            "%a, %d %b %y %H:%M:%S %z",
            "%a, %d %b %Y %H:%M:%S %z",
            "%Y-%m-%dT%H:%M:%S%z",
...
if __name__ == "__main__":
    ...
    print(f"=== AMD News ({len(result.get('news', []))} items) ===")
